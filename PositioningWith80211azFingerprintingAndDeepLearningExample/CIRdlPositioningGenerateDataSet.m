function [cir, labels] = CIRdlPositioningGenerateDataSet(rays, STAs, APs, cfg, snrs)
...
numChan = numel(rays);
txWaveform = single(CIRheRangingWaveformGenerator(cfg)); % Generate 802.11az packet
tx0=CIRheRangingWaveformGenerator(cfg);
heRangingWavGenPlot(tx0,cfg);
